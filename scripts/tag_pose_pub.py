...
class TagPosePub:
    def __init__(self):
        ...
        self.tag_pub2 = rospy.Publisher('world_tag_poses', MarkerArray, queue_size=10)
        self.tag_sub = rospy.Subscriber('/dragonfly21/dfc/apriltag_pose_estimator/apriltag_poses', ApriltagPoseStamped, self.tag_cb)
        self.tag_sub2 = rospy.Subscriber('/dragonfly21/world_apriltag_poses', ApriltagPoseStamped, self.tag_cb2)
    ...
    def tag_cb2(self, data):
        marker_array = MarkerArray()
        for i in range(len(data.apriltags)):
            marker = Marker()
            marker.header.frame_id = "global_origin"
            marker.header.stamp = data.posearray.header.stamp
            marker.ns = 'apriltag'
            marker.id = data.apriltags[i].id
            marker.type = Marker.CUBE
            ...
            marker.color.b = 0.0
            marker_array.markers.append(marker)
        self.tag_pub2.publish(marker_array)
